...
namespace MembershipApps.Extensions
{
    public static class SectionExtensions
    {
        public static async Task<ProductSectionModel> GetProductSectionsAsync(int productId, string userId)
        {
            ...
            {
                section.Items = await GetProductItemAsync(productId, section.Id, section.ItemTypeId, userId);
            }
        ...
        public static async Task<IEnumerable<ProductItemRow>> GetProductItemAsync(
            ...
            var items = await (from i in db.Items
                ...
                select new ProductItemRow()
                {
                    ...
                    IsAvailable = DbFunctions.CreateDateTime(today.Year,
                                      today.Month, today.Day, 0, 0, 0) >= DbFunctions.CreateDateTime(
                                      us.StartDate.Value.Year,
                                      us.StartDate.Value.Month, us.StartDate.Value.Day + i.WaitDays, 0, 0, 0),
